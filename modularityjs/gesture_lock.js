...
module.exports = class {
  constructor(canvasid, context, cb, opt) {
    ...
    this.marge = this.margeCircle = 0; //触摸点及触摸点和画布边界间隔
    this.initColor = opt && opt.initColor || 'black';
    this.checkColor = opt && opt.checkColor || 'black';
    this.errorColor = opt && opt.errorColor || 'black';
  ...
  calCircleParams() {
    ...
    //     }
    // }
    let touchPoint = [{ x: 90, y: 60, index: 0, check: "uncheck" }, { x: 180, y: 120, index: 1, check: "uncheck" }, { x: 300, y: 60, index: 2, check: "uncheck" }]
    this.touchPoints = this.touchPoints.concat(touchPoint)
